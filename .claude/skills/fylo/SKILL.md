...
name: fylo
description: >
  Build, run, test, and extend the Fylo unified file-sharing platform
...
# Fylo — build, run, verify

Java 25 unified file-sharing backend (4 modes, ONE engine/API/auth/storage/DB).
Evolved from PeerLink; package root is `p2p.*` (`com.fylo` rename deliberately
deferred). Branch: `fylo-unified-platform`.
